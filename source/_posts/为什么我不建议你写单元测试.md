...
**很**明显，你已经抢了 Tester 的饭碗！你知道 bug 对于 Tester 意味着什么吗？这是 KPI 啊，是工作效率啊。当测试你写的程序的时候，bug 却寥寥无几，你知道他的内心有多麽焦虑，多麽惶恐。这让会让他感到深深的挫败感，你也应该为此感到内疚。

当其他开发同学都在忙着修 bug 的时候，而你却守着 80% 几的测试覆盖率，在一边喝着茶水，一边和前台妹妹打趣，这会使同僚心生妒忌，从此结下梁子。日后你再也没有机会与这群汉子一起吃饭，只能和妹子共进晚餐，真是太惨了！！！
...
**注**：这里使用 [AVA](https://github.com/avajs/ava) 作为测试框架，断言使用其自带的 `t.is`，而非常见的 `Assert` ，并不太大区别。
...
单元测试的具备独立性表现为两种：函数内部的独立，单元测试的独立。
...
显然，这个单元测试运行*缓慢*且不能*通过*，因为依赖网络和外部数据。在这个场景下我们只想测试 Top 3 的功能是否正确，只要返回正确的 3 个值就ok，这时候可以用上我们的 [sinon.js](https://sinonjs.org/) 库。
...
随着工具的发展，Fake 的**实用性已经降低**  ，大多数时候并不需要我们手动去制作 Fake 对象。
...
如果 `a()` `b()` `c()` 都被 Mock 掉，那么测试 `getResult` 的意义就没有了，当这几个函数的内部被修改甚至无法正确返回，我们的测试无法不会测出来。所以答应我，只用它隔离副作用好吗？

## 总结

为了你可以成功避开正确的单元测试，我也是拼了老命把单元测试的正确技巧都罗列了一遍。单元测试并不复杂，但想要精巧的避开还需要多加练习，为此我还特意创建了 [git 仓库](https://github.com/xbl/unit-test-ts)，包含以上所有示例。
